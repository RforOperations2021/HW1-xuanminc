    ...
    sidebarLayout(
        ...
        mainPanel(
            tabsetPanel(type = "tab",
                        tabPanel("Global Situation", DT::dataTableOutput(outputId = "globalTable")),
                        tabPanel("Scatter Plot", plotOutput("scatterplot")),
                        tabPanel("Histogram", plotOutput("histogram1")),
...
server <- function(input, output) {
    ...
    output$scatterplot <- renderPlot({
        ggplot(data = df[2:nrow(data),], aes(x = Deaths_cumulative_total, y = Cases_cumulative_total, color = WHO_Region)) +
            ...
            labs(x = "Deaths_cumulative_total",
    ...
    output$histogram1 <- renderPlot({
        ggplot(data = df[df$WHO_Region == input$region,], aes(x = Cases_cumulative_total, fill = input$region)) +
            geom_histogram() +
            labs(x = "Cases_cumulative_total")
    })
    
    # create box plot
    output$boxplot <- renderPlot({
        ggplot(data[data$Transmission_Classification %in% c("Community transmission","Clusters of cases","No cases","Sporadic cases"),], aes(x = as.factor(Transmission_Classification), y = Cases_newly_reported_in_last_7_days)) +
